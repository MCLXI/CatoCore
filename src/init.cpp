...
void Shutdown()
{
    ...
    RenameThread("PentaNode-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: PentaNode.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: PentaNoded.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 8557)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 8558)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePentaNodeamount=<n> " + _("Keep N PentaNode anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. PentaNode is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PentaNode is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("PentaNode version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "PentaNode server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of PentaNode") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart PentaNode to complete") << "\n";
    ...
    nAnonymizePentaNodeAmount = GetArg("-anonymizePentaNodeamount", 0);
    if(nAnonymizePentaNodeAmount > 999999) nAnonymizePentaNodeAmount = 999999;
    if(nAnonymizePentaNodeAmount < 2) nAnonymizePentaNodeAmount = 2;
    ...
    LogPrintf("Anonymize PentaNode Amount %d\n", nAnonymizePentaNodeAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1PTN+1000 == (.1PTN+100)*10
       10PTN+10000 == (1PTN+1000)*10
    */
