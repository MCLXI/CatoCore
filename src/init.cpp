...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: CatoCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: CatoCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeCatoCoinamount=<n> " + _("Keep N CatoCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CatoCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CatoCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CatoCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CatoCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart CatoCoin to complete") << "\n";
    ...
    nAnonymizeCatoCoinAmount = GetArg("-anonymizeCatoCoinamount", 0);
    if(nAnonymizeCatoCoinAmount > 999999) nAnonymizeCatoCoinAmount = 999999;
    if(nAnonymizeCatoCoinAmount < 2) nAnonymizeCatoCoinAmount = 2;
    ...
    LogPrintf("Anonymize CatoCoin Amount %d\n", nAnonymizeCatoCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1CATO+1000 == (.1CATO+100)*10
       10CATO+10000 == (1CATO+1000)*10
    */
