...
int nAnonymizePentaNodeAmount = 1000;
...
#else
    const char* pszModule = "PentaNode";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PentaNode
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PentaNode
    // Mac: ~/Library/Application Support/PentaNode
    // Unix: ~/.PentaNode
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PentaNode";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "PentaNode.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "PentaNoded.pid"));
