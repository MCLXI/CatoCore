...
int nAnonymizeCatoCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CatoCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CatoCoin
    // Mac: ~/Library/Application Support/CatoCoin
    // Unix: ~/.CatoCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CatoCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "CatoCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "CatoCoind.pid"));
