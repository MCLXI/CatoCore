...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePentaNodeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePentaNodeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePentaNodeAmount = strAnonymizePentaNodeAmount.remove(strAnonymizePentaNodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePentaNodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePentaNodeAmount*COIN) nMaxToAnonymize = nAnonymizePentaNodeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePentaNodeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePentaNodeAmount));
        strAnonymizePentaNodeAmount = strAnonymizePentaNodeAmount.remove(strAnonymizePentaNodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePentaNodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizePentaNodeAmount == 0){
