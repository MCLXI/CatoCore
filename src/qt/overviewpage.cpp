...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCatoCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCatoCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCatoCoinAmount = strAnonymizeCatoCoinAmount.remove(strAnonymizeCatoCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCatoCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCatoCoinAmount*COIN) nMaxToAnonymize = nAnonymizeCatoCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCatoCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCatoCoinAmount));
        strAnonymizeCatoCoinAmount = strAnonymizeCatoCoinAmount.remove(strAnonymizeCatoCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCatoCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
